...
|---|---|---|
| **Foundry Local** | no Azure Foundry mode selected and `AiServices:DisableFoundryLocal=false` | The AppHost wires no local `chat` resource. The bootstrapper uses `Microsoft.AI.Foundry.Local` directly, starts the OpenAI-compatible local endpoint, records `AiProviderInfo("local")`, and registers it as the shared `IChatClient`. If the local SDK cannot bootstrap, the bootstrapper logs the failure and falls back to no-op AI. |
...
| **Disabled** | `AiServices:DisableFoundryLocal=true`, or local bootstrap unavailable and no Azure setting | No provider is active. `TaskFlow.Infrastructure.AI` registers `AiProviderInfo("none")` and `NoOpChatClient`, so the app starts without model credentials. |
