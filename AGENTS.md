...
| Tests | `src/Test/` (15 projects; see REFERENCE-STATUS.md for verified counts) |

## Build and test

```powershell
dotnet build src/TaskFlow.slnx                                 # 0 warnings expected
dotnet build src/UI/TaskFlow.Uno/TaskFlow.Uno.csproj           # Uno builds separately (Uno.Sdk)
dotnet test src/TaskFlow.slnx --filter "TestCategory=Unit"     # fast lane
dotnet test src/TaskFlow.slnx --filter "TestCategory=Architecture|TestCategory=Endpoint"
